...
export const FormField = ({ label, name, id, type = 'text', min, value, onChange }: FormFieldProps) => {
	return (
		<Wrapper>
			<StyledLabel htmlFor={name}>{label}</StyledLabel>
			<StyledInput name={name} id={id} type={type} min={min} value={value} onChange={onChange} data-testid={label} />
		</Wrapper>
	);
};
